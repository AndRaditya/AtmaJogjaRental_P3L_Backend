...
class DriverController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validate = Validator::make($storeData, [
            ...
            'email_driver' => 'required|email:rfc|unique:driver_10144s',
    ...
    public function update(Request $request, $id_driver_increment)
    {
        ...
        $total_rating = DB::table('detail__transaksi__mobil_10144s')
            ->where('id_driver_increment', $id_driver_increment)
            ->sum('rating_driver');
        $count_rating = DB::table('detail__transaksi__mobil_10144s')
            ->where('id_driver_increment', $id_driver_increment)
            ->count('rating_driver');

        if ($id_driver_increment != 7) {
            if ($count_rating == 0) {
                $temp_rerata_rating = 0;
            } else {
                $temp_rerata_rating = $total_rating / $count_rating;
            }
        }
        ...
        $driver->rerata_rating = $temp_rerata_rating;
    ...
    public function updateDriverMobile(Request $request, $id_driver_increment)
    {
        $driver = Driver_10144::where('id_driver_increment', $id_driver_increment)->first();
        if (is_null($driver)) {
            return response([
        ...
        $validate = Validator::make($updateData, [
            ...
            'nomor_telepon_driver' => 'nullable',
            'alamat_driver' => 'nullable',
            'email_driver' => 'nullable',
    ...
    public function updateStatusDriverMobile(Request $request, $id_driver_increment)
    {
        $driver = Driver_10144::where('id_driver_increment', $id_driver_increment)->first();
        if (is_null($driver)) {
            return response([
